...
public class CardPreviewInfo : MonoBehaviour
{
    ...
    public void SetCardPreview(DBHero heroInfo, long frame, bool isFullPreview)
    {
        ...
        //Debug.Log(info.name);
        //Debug.Log(info.description.Count);
        ...
        if (info != null)
        {
            if (isFullPreview)
            {
                ...
                {
                    description.text = "";
                    // full info ngoai battle chi hien thi key tren bai k hien thi mo ta cua keyword
                    UpdateHeroInfo(heroInfo);
                    if (info.description.Count > 0)
                    {
                        //Debug.Log("herrooo " + info.id + ":" + info.description[0]);
                        if (heroInfo.type != DBHero.TYPE_GOD)
                        {
                            string txt = "";
    ...
    private void UpdateHeroInfoFull(DBHero hero)
    {
        ...
        if (hero != null)
        {
            ...
            if (hero.godSlayer > 0)
            {
                ...
                trans.SetParent(keyWordContainer);
                trans.localScale = Vector3.one;
                trans.GetComponent<KeywordInfo>().InitData(txt, "God Slayer", hero.godSlayer);
            }
            if (hero.cleave > 0)
            {
                string txt = "";
    ...
    private void SetMinionPreviewCard(DBHero hero, long frame)
    {
        ...
        if (frame >= 3)
        {
            ...
            cardPrintImage.GetComponent<RectTransform>().sizeDelta = new Vector2(520, 700);
        }
        else
        {
            cardPrintImage.sprite = CardData.Instance.GetOnBoardSprite(hero.id);
            cardPrintImage.GetComponent<RectTransform>().sizeDelta = new Vector2(670, 670);
            Texture mask = CardData.Instance.GetCardFrameMaskTexture("M_" + hero.type + "_" + /*hero.rarity*/1);
            if (effPowers.childCount == 4)
            {
                effPowers.GetChild(0).gameObject.SetActive(true);
                effPowers.GetChild(0).GetComponent<Image>().material = Instantiate(CardData.Instance.GetCardFrameMaterial("Mat_EP_" + hero.type + "_" + 1));
