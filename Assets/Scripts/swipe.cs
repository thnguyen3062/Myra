...
public class swipe : MonoBehaviour
{
            ...
            for (int i = 0; i < pos.Length; i++)
            {
                if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2))
                {
                    ...
                    if(leftBut!= null && rightBut!= null)
                    {
                        if(i==0)
                        {
                            leftBut.GetComponent<Image>().color = new Color(.5f, .5f, .5f, .5f);
                        }
                        else if(i==pos.Length-1)
                        {
                            rightBut.GetComponent<Image>().color = new Color(.5f, .5f, .5f, .5f);
                        }    
                        else
                        {
                            leftBut.GetComponent<Image>().color = new Color(1,1,1,1);
                            rightBut.GetComponent<Image>().color = new Color(1,1,1,1);
                        }    
                    }    
                }
            }
    ...
    public void SetPos(int index)
    {
        ...
        for (int i = 0; i < pos.Length; i++)
        {
            if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2))
            {
                scrollbar.GetComponent<Scrollbar>().value = Mathf.Lerp(scrollbar.GetComponent<Scrollbar>().value, pos[index], 1f * Time.deltaTime);
                if (leftBut != null && rightBut != null)
                {
                    if (i == 0)
                    {
